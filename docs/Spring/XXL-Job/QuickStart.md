...
调度中心是一个Springboot项目，按照Springboot项目的部署流程即可
...
使用`@XxlJob`向XxlJobSpringExecutor配置一个Handler
...
`XxlJobSpringExecutor`会根据配置的调度中心地址向调度中心注册，并启用一个内部服务器占用端口用于接收调度中心请求
