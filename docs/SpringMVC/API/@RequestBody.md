...
当一个请求打到带有 @RequestBody 的 Controller 方法时，Spring 的工作流程：

1. **识别注解**：Spring 看到参数上有 `@RequestBody`，就知道这个参数的数据不在 URL 中，也不在表单（Form Data）里，而是在 HTTP 的Body中。
...
GET 请求里用 `@RequestBody` 基本是没用的（也不推荐）
